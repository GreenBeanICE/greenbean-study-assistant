...
def test_workspace_accepts_default_and_custom_types():
    ...
    workspace = Workspace(name="French Admin", type="caf")
...
def test_document_unit_supports_rag_metadata_and_parser_provenance():
    unit = DocumentUnit(
        ...
        sequence_index=3,
        text_content="The assignment is due next Friday.",
        page_number=2,
        start_char=120,
        end_char=158,
        token_count=8,
...
def test_section_supports_header_hierarchy_and_parser_metadata():
    ...
    child = Section(
        ...
        level=2,
        order_index=1,
        metadata_json={"header_path": ["Course Overview", "Assignments"]},
    )

    assert_uuid(parent.id)
    assert child.parent_section_id == parent.id
    assert child.metadata_json["header_path"] == ["Course Overview", "Assignments"]
...
def test_analysis_result_supports_full_document_and_section_analysis():
    ...
    section = AnalysisResult(
        ...
        content_json={"key_points": ["deadline"]},
...
def test_chat_session_scopes_to_workspace_or_document_only():
    workspace_session = ChatSession(workspace_id="workspace_1", title="General questions")
...
def test_chat_message_role_is_user_or_agent():
    ...
    agent_message = ChatMessage(
        ...
        content="The homework is due Friday.",
