...
- `DocumentUnit`：文档解析后的内容单元，保存顺序、正文、页码、字符范围、token 数、解析元数据和原始布局/OCR 信息。
- `Section`：文档章节结构，支持父子层级、页码范围、摘要、解析来源元数据。
- `SectionUnitLink`：章节和内容单元的多对多关联，要求持久化层保证同章节内关联和排序唯一。
- `EmbeddingVector`：内容单元向量，校验 `vector` 长度必须等于 `vector_dimension`。
